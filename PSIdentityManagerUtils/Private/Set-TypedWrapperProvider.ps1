  ...
  Process
  {
    try {
      ...
      ForEach ($tableProperty in $tables) {
        ...
        Write-Debug "Generate Set-$Prefix$funcName"
...
$([Environment]::NewLine)
  Process {
    try {
      ...
      Set-Entity -Session `$session -Entity `$Entity -Type '$funcName' -Identity `$Identity -Properties `$properties -Unsaved:`$Unsaved
    } catch {
